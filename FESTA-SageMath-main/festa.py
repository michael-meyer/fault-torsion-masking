...
class FESTA:
    ...
    def _trapdoor_eval(self, pk, s1, s2, B, fault_coeff=None, fault_bit=1):
        """
        ...
        R1, S1 = mask_torsion_points(B, imPb, imQb, fault_coeff, fault_bit=fault_bit)
    ...
    def encrypt(self, pk_compressed, m, fault_coeff=None, fault_bit=1): # FAULT INJECTED HERE
        """
        ...
        c = self._trapdoor_eval(pk, s, t, T, fault_coeff, fault_bit=fault_bit)
        # c_bytes = self.compress_ciphertext(c)
        c_bytes = c # SIMPLIFIED VERSION, NO COMPRESSION
    ...
    def decrypt(self, c_bytes):
        ...
        """
        # The fault simulation keeps the ciphertext uncompressed.
        # The reference compressed representation is also accepted.
        if isinstance(c_bytes, tuple) and len(c_bytes) == 6:
            c = c_bytes
        else:
