...
# This file was *autogenerated* from the file lvl5_paper_attack.sage
from sage.all_cmdline import *   # import sage library

_sage_const_5 = Integer(5); _sage_const_1p0 = RealNumber('1.0'); _sage_const_30 = Integer(30); _sage_const_0p0 = RealNumber('0.0'); _sage_const_100p0 = RealNumber('100.0'); _sage_const_60p0 = RealNumber('60.0'); _sage_const_3 = Integer(3); _sage_const_0 = Integer(0); _sage_const_4 = Integer(4); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_32 = Integer(32); _sage_const_8 = Integer(8)
import os
import sys
import time
...
def show_progress(done, total, started):
    ...
    ratio = float(done) / total if total else _sage_const_1p0 
    width = _sage_const_30 
    filled = min(width, int(width * ratio))
    bar = "#" * filled + "-" * (width - filled)
    eta = elapsed * (total - done) / done if done else _sage_const_0p0 
    print("\r    [%s] %6.2f%%  %d/%d  elapsed %5.1f min  ETA(max) %5.1f min" %
          (bar, _sage_const_100p0  * ratio, done, total, elapsed / _sage_const_60p0 , eta / _sage_const_60p0 ),
          end="", flush=True)
...
def reverse_chains(EB):
    ...
    for _ in range(DEPTH):
        ...
        for line, previous_j, chain in states:
            curve = line.curve()
            P, Q = torsion_basis(curve, _sage_const_3 )
            for K in (P, Q, P + Q, P - Q):
                phi = KummerLineIsogeny(line, line(K[_sage_const_0 ]), _sage_const_3 )
...
def prefix_images(H, images, chain, A):
    ...
    if (P - Q)[_sage_const_0 ] != xPQ.x():
        Q = -Q
    inv = ZZ(inverse_mod(_sage_const_3 **DEPTH, _sage_const_4 *A))
    return inv*P, inv*Q
...
def recover_r(H, theta4, F, Epre, EB, q):
    ...
    P, Q = _sage_const_3 **DEPTH*H["PB"], _sage_const_3 **DEPTH*H["QB"]
    U, V = H["eval_two_consistent"](
        theta4, F, H["E0"], Epre, P, Q, component=_sage_const_2 
    )
    r0 = ZZ(ell_discrete_log_pari(Epre, -U, V, q)) % q

    U, V = H["eval_two_consistent"](
        theta4, F, H["E0"], Epre, H["PB"], H["QB"], component=_sage_const_2 
    )
    line = KummerLine(Epre)
    answers = []
    for high in range(_sage_const_3 **DEPTH):
        r = ZZ(r0 + high*q)
        K = U + r*V
        if K.is_zero() or _sage_const_3 **(DEPTH - _sage_const_1 )*K == Epre(_sage_const_0 ):
            continue
        tail = KummerLineIsogeny(line, line(K[_sage_const_0 ]), _sage_const_3 **DEPTH)
...
def decrypt_with_r(H, pk, ct, r):
    ...
    if (X - Y)[_sage_const_0 ] != psi(H["xXY_0"]).x():
        Y = -Y
    ...
    X = psi_prime(EA(X_A[_sage_const_0 ])).curve_point()
    Y = psi_prime(EA(Y_A[_sage_const_0 ])).curve_point()
    if (X - Y)[_sage_const_0 ] != psi_prime(EA((X_A - Y_A)[_sage_const_0 ])).x():
        Y = -Y
    X, Y = d1*X + d2*Y, d3*X + d4*Y
    return H["xof_encrypt"](H["xof_kdf"](X[_sage_const_0 ], Y[_sage_const_0 ]), ct_bytes)


def attack(seed=_sage_const_1 ):
    t0 = time.time()
    ...
    print("[4/6] Enumerating 324 reverse paths and preparing Kani parameters...",
          flush=True)
    chains = reverse_chains(ct[_sage_const_0 ])

    q = ZZ(_sage_const_3 **(H["b"] - DEPTH))
        ...
        for Epre_line, chain in chains:
            ...
            try:
                ...
                H["eval_two_consistent"](
                    ...
                    H["X0"], H["Y0"], component=_sage_const_2 
                )
                answers = recover_r(H, theta4, F, Epre_line.curve(), ct[_sage_const_0 ], q)
            except (AssertionError, ValueError, ZeroDivisionError):
                pass
            show_progress(checked, total_candidates, search_started)
            if len(answers) == _sage_const_1 :
                recovered_r = answers[_sage_const_0 ]
                break
...
if __name__ == "__main__":
    attack(int(sys.argv[_sage_const_1 ]) if len(sys.argv) > _sage_const_1  else _sage_const_1 )
