...
class GluingThetaIsogeny(ThetaIsogeny):
    ...
    def __call__(self, P, fp4=False):
        ...
        if fp4:
            EP = self.T_shift[0].curve()
            GFp = EP.base_ring()
            p = GFp.characteristic()
            GPf4 = GFp.extension(2, 'zz')

            # print(EP)

            EP_ = EP.change_ring(GPf4)
            # print(EP_)

            # print("P", P[0][0])
            print("t_shift", self.T_shift[0][0])

            print(P[0].curve())
            print(EP_)

            # P0_ = EP_(P[0])
            T_shift_ = EP_(self.T_shift[0])

            P_sum_T_0 = P[0] + T_shift_
            
            print(P_sum_T_0.xy()[0])
            ...
            # print(P)
            # print("T_shift", self.T_shift)
            # print(self.T_shift[0])
            # print(self.T_shift[0].order())
